import { setores, questionarios, avaliacoes, Avaliacao, Questionario } from './mockDatabase';
 ...
 */
export const calcularPerformanceParaGrafico = (filters: PerformanceFilters = {}): ChartDataItem[] => {
    ...
    if (filters.tipo) {
        const idsSetoresFiltrados = setores
            ...
            .map(setor => setor.id);
        avaliacoesFiltradas = avaliacoes.filter(aval => idsSetoresFiltrados.includes(aval.setorId));
    } else if (filters.setorId) {
        avaliacoesFiltradas = avaliacoes.filter(aval => aval.setorId === filters.setorId);
    }
    ...
    const resultadoFinal = questionarios.map(questionario => {
        ...
        if (totais && totais.totalPontosProjetado > 0) {
            percentual = (totais.totalPontosAtual / totais.totalPontosProjetado) * 100;
        }
...
console.log("--- DADOS PARA GRÁFICO (VISÃO GERAL) ---");
...
console.log('\n--- DADOS PARA GRÁFICO (FILTRO: "Internação") ---');
const dadosGraficoInternacao = calcularPerformanceParaGrafico({ tipo: 'Internacao' });
console.table(dadosGraficoInternacao);

// Cenário 3: Filtrando por setor "UTI Adulto"
console.log('\n--- DADOS PARA GRÁFICO (FILTRO: "UTI Adulto") ---');
const dadosGraficoUtiAdulto = calcularPerformanceParaGrafico({ setorId: 'uti-adulto' });
