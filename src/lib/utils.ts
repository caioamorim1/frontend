...
export function formatAmountBRL(amount: number): string {
  ...
  if (amount >= 1_000_000) {
    ...
    // Formata com 1 casa decimal e troca o ponto por vírgula
    const formattedValue = valueInMillions.toFixed(2).replace('.', ',');
  ...
  if (amount >= 1_000) {
    ...
    // Formata com 1 casa decimal e troca o ponto por vírgula
    const formattedValue = valueInThousands.toFixed(2).replace('.', ',');
