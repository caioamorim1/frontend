...
export default function ColetasPage() {
  ...
  const handleStartColeta = () => {
    ...
    const unidadeNome =
      unidades.find((u) => u.id === selectedUnidade)?.nome || "desconhecida";
    navigate(
      `/coletas/realizar/${selectedQuestionario}/${selectedUnidade}/${encodeURIComponent(
        unidadeNome
      )}`
    );
  };
  ...
  if (loading)
    return <p className="text-center p-10">Carregando dados de coletas...</p>;
    ...
    <div className="space-y-8">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <PlusCircle className="text-primary" /> Iniciar Nova Coleta de Dados
          </CardTitle>
          <CardDescription>
            Selecione a unidade e o questionário que deseja aplicar.
          </CardDescription>
        </CardHeader>
        <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <Button
            ...
            disabled={!selectedUnidade || !selectedQuestionario}
          >
            Iniciar Avaliação
          </Button>
        </CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Visualize as avaliações já realizadas neste hospital.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
