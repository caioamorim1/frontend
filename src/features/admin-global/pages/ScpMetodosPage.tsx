...
export default function ScpMetodosPage() {
  ...
  const validateFaixas = (faixas: any[]): { ok: boolean; message?: string } => {
    ...
    const normalized = faixas.map((f) => ({ min: Number(f.min), max: Number(f.max), classe: String(f.classe) }))
      .sort((a, b) => a.min - b.min);

    for (let i = 0; i < normalized.length; i++) {
      const f = normalized[i];
      if (!Number.isFinite(f.min) || !Number.isFinite(f.max)) return { ok: false, message: 'Faixas precisam ter min e max numéricos.' };
      if (f.min > f.max) return { ok: false, message: 'Em uma faixa, min não pode ser maior que max.' };
      if (i > 0) {
        const prev = normalized[i - 1];
        // require contiguous coverage: prev.max + 1 === f.min
        if (f.min !== prev.max + 1) return { ok: false, message: 'As faixas devem ser contínuas e não se sobrepor. Verifique min/max.' };
      }
    }
    return { ok: true };
  };

  const validateQuestions = (questions: any[]): { ok: boolean; message?: string } => {
    if (!questions) return { ok: true };
    const keys = new Set<string>();
    for (const q of questions) {
      const key = String(q.key || '').trim();
      if (!key) return { ok: false, message: 'Cada questão precisa ter uma key (identificador).' };
      if (/\s/.test(key)) return { ok: false, message: `Key da questão '${key}' não pode conter espaços.` };
      const lower = key.toLowerCase();
      if (keys.has(lower)) return { ok: false, message: `Keys de questões precisam ser únicas: duplicado '${key}'.` };
      keys.add(lower);
      // validate options
      if (!Array.isArray(q.options) || q.options.length === 0) return { ok: false, message: `Questão '${key}' precisa ter pelo menos uma opção.` };
      for (const opt of q.options) {
        if (opt.label === undefined || opt.label === null) return { ok: false, message: `Opção sem label na questão '${key}'.` };
        if (!Number.isFinite(Number(opt.value))) return { ok: false, message: `Opção '${opt.label}' na questão '${key}' precisa ter value numérico.` };
      }
    }
    return { ok: true };
  };
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const payload: CreateScpMetodoDTO = {
      ...
      questions: (formData.questions || []).map((q: any) => ({
        ...
        text: String(q.text || ''),
        options: (q.options || []).map((opt: any) => ({ label: String(opt.label || ''), value: Number(opt.value) })),
      })),
      faixas: (formData.faixas || []).map((f: any) => ({ min: Number(f.min), max: Number(f.max), classe: String(f.classe) })),
    };
  ...
  const removeQuestion = (index: number) => {
    setFormData(prev => {
      const questions = (prev.questions || []).filter((_: any, i: number) => i !== index);
      return { ...prev, questions };
    });
  };
  ...
  const addQuestionOption = (qIndex: number) => {
    setFormData(prev => {
      const questions = (prev.questions || []).map((q: any, i: number) => {
        ...
        return { ...q, options: [...(q.options || []), { label: '', value: 0 }] };
      });
      return { ...prev, questions };
    });
  };

  const updateQuestionOption = (qIndex: number, optIndex: number, patch: Partial<any>) => {
    setFormData(prev => {
      const questions = (prev.questions || []).map((q: any, i: number) => {
        if (i !== qIndex) return q;
        const options = (q.options || []).map((o: any, oi: number) => (oi === optIndex ? { ...o, ...patch } : o));
        return { ...q, options };
      });
      return { ...prev, questions };
    });
  ...
  const removeQuestionOption = (qIndex: number, optIndex: number) => {
    setFormData(prev => {
      const questions = (prev.questions || []).map((q: any, i: number) => {
        if (i !== qIndex) return q;
        const options = (q.options || []).filter((_: any, oi: number) => oi !== optIndex);
        return { ...q, options };
      });
      return { ...prev, questions };
    });
  ...
  const updateFaixa = (index: number, patch: Partial<any>) => {
    setFormData(prev => {
      const faixas = (prev.faixas || []).map((f: any, i: number) => (i === index ? { ...f, ...patch } : f));
      return { ...prev, faixas };
    });
  };

  const removeFaixa = (index: number) => {
    setFormData(prev => ({ ...prev, faixas: (prev.faixas || []).filter((_: any, i: number) => i !== index) }));
  };
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {formData.questions?.map((q: any, i: number) => (
                  <div key={i} className="p-3 bg-gray-50 rounded space-y-2">
                    <div className="flex items-center gap-2">
                      ...
                      />
                      <button type="button" onClick={() => removeQuestion(i)} className="text-red-600">Remover</button>
                    </div>
                    <input
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center justify-between">
                        ...
                        <button type="button" onClick={() => addQuestionOption(i)} className="text-sm text-secondary">Adicionar Opção</button>
                      </div>
                      {(q.options || []).map((opt: any, oi: number) => (
                        <div key={oi} className="flex gap-2">
                          <input className="flex-1 p-1 border rounded" placeholder="Label" value={opt.label || ''} onChange={(e) => updateQuestionOption(i, oi, { label: e.target.value })} />
                          <input className="w-16 p-1 border rounded" placeholder="Valor" type="number" value={opt.value ?? 0} onChange={(e) => updateQuestionOption(i, oi, { value: Number(e.target.value) })} />
                          <button type="button" onClick={() => removeQuestionOption(i, oi)} className="text-red-600">x</button>
                        </div>
                      ))}
                    </div>
                  </div>
             ...
             <div className="space-y-2">
                ...
                {formData.faixas?.map((f: any, i: number) => (
                  <div key={i} className="p-3 bg-gray-50 rounded space-y-2">
                    <div className="flex items-center gap-2">
                      <select value={f.classe || 'MINIMOS'} onChange={(e) => updateFaixa(i, { classe: e.target.value })} className="p-2 border rounded">
                      ...
                      <button type="button" onClick={() => removeFaixa(i)} className="text-red-600">Remover</button>
                    </div>
                    <div className="flex gap-2">
                      <input className="flex-1 p-2 border rounded" type="number" value={f.min ?? 0} onChange={(e) => updateFaixa(i, { min: Number(e.target.value) })} />
                      <input className="flex-1 p-2 border rounded" type="number" value={f.max ?? 0} onChange={(e) => updateFaixa(i, { max: Number(e.target.value) })} />
                    </div>
                  </div>
                ))}
             </div>
