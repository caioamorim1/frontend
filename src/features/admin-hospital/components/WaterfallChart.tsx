...
const CustomizedLabel: React.FC<any> = (props) => {
    ...
    return (
        <text x={x + width / 2} y={y - 6} fill="hsl(var(--foreground))" textAnchor="middle" dominantBaseline="middle" fontSize={12}>
            {formattedValue}
        </text>
    );
};

const ReusableWaterfall: React.FC<{ data: WaterfallDataItem[], unit: 'currency' | 'people', title: string }> = ({ data, unit, title }) => {
...
const GlobalTabContent: React.FC<{ global: WaterfallDataItem[], vsInternacao: WaterfallDataItem[], vsNaoInternacao: WaterfallDataItem[] }> = ({ global, vsInternacao, vsNaoInternacao }) => {
    ...
    return (
        <div className='space-y-4'>
            ...
            <ReusableWaterfall data={dataMap[view]} unit="currency" title="Análise de Custos" />
        </div>
    );
}
...
const DetailTabContent: React.FC<{ detailedData: DetailedWaterfallData }> = ({ detailedData }) => {
    ...
    const chartDataQuantitativo = detailedData[selectedSector]?.['quantitativo'] || [];
    ...
    return (
        <div className="space-y-4">
            ...
            <div style={{ display: 'flex', flexDirection: 'row', gap: '1.5rem' }}>
                <ReusableWaterfall data={chartDataCusto} unit={'currency'} title="Análise de Custos" />
                <ReusableWaterfall data={chartDataQuantitativo} unit={'people'} title="Análise de Pessoal" />
            </div>
        </div>
    );
};
