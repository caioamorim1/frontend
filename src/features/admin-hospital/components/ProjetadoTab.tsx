...
export default function ProjetadoTab({ unidade }: ProjetadoTabProps) {
    ...
    const [analiseBase, setAnaliseBase] = useState<LinhaAnalise[]>([]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                toast({ variant: 'destructive', title: 'Erro', description: 'Não foi possível carregar os dados para o ajuste projetado.' });
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            toast({ title: "Sucesso!", description: "Ajustes qualitativos salvos com sucesso." });
        } catch (error) {
            toast({ variant: 'destructive', title: 'Erro', description: 'Não foi possível salvar os ajustes.' });
        } finally {
            setSaving(false);
        }
    };
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow>
                                ...
                                <TableHead className="text-center">Projetado (Sistema)</TableHead>
                                <TableHead className="text-center w-[200px]">Ajuste Qualitativo</TableHead>
                            ...
                            {analiseBase.map(linha => {
                                ...
                                return (
                                    <TableRow key={linha.cargoId}>
                                        ...
                                        <TableCell className="text-center font-bold text-xl text-primary">{projetadoFinal}</TableCell>
                                    </TableRow>
                                )
                            })}
                        </TableBody>
