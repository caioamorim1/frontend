        ...
        <Card>
            ...
            <CardContent>
                <ResponsiveContainer width="100%" height={340}>
                    <BarChart data={processed} margin={{ top: 40, right: 20, left: 0, bottom: 5 }}>
                        ...
                        <Bar dataKey="range">
                            ...
                            <LabelList
                                ...
                                content={({ x, y, width, height, value, index }) => {
                                    ...
                                    const isFixedBar =
                                        role === "Atual" || role === "Projetado" || role === "Baseline";

                                    const diff = Number(value); // converte para número
                                    const end = Number(processed[index].end); // garante tipo number
...
const CustomTooltip = ({
    ...
    const isFixed =
        entry.role === "Atual" || entry.role === "Projetado" || entry.role === "Baseline";
    ...
    const arrowColor = isPositive ? "text-red-600" : isNegative ? "text-green-600" : "";
    ...
    return (
        <div className="bg-background border p-2 rounded shadow text-sm">
            ...
            <p className={`font-semibold  ${isFixed ? "" : arrowColor}`}>
                {isFixed ? format(Math.abs(value)) : `${arrow} ${format(Math.abs(value))}`}
            </p>
        </div>
    );
};
